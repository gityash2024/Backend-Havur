...
export const remove = async (req: Request, res: Response) => {
  ...
  await IpdPatient.deleteOne({ _id: req.body.id, hospitalId: req.body.hid });

  res.status(200).json({ message: "IPD patient deleted successfully." });
};

export const associatedIpdList = async (req: Request, res: Response) => {
  const ipdPatient = await IpdPatient.find({
