...
import { Patients } from "./_validation";
...
export const view = async (req: Request, res: Response) => {
  let patient: any = await Patients.findOne({ _id: req.body.pid }).select({
    password: 0,
    verificationCode: 0,
    status: 0,
  });
...
export const update = async (req: Request, res: Response) => {
  ...
  let isEmailExist: any = await Patients.findOne({
...
export const viewMfile = async (req: Request, res: Response) => {
  ...
  let patient: any = await Patients.findOne({ _id: pid }).select({ mFile: 1 });
  if (!patient) return res.status(400).json({ message: "No record found." });
