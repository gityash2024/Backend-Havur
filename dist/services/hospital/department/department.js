...
const add = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let department = yield _validation_1.Department.findOne({
        name: req.body.name,
    });
    if (department)
        return res.status(400).json({ message: "Department already exist." });
    let departments = new _validation_1.Department(lodash_1.default.pick(req.body, ["name"]));
    departments.createdAt = new Date().toISOString();
    departments.updatedAt = new Date().toISOString();
    departments = yield departments.save();
    res.status(200).json({ message: "Department added successfully." });
});
exports.add = add;
const list = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const department = yield _validation_1.Department.find()
        .select({
        status: 0,
    })
        .sort({ createdAt: -1 })
